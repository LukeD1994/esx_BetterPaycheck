...
AddEventHandler('playerDropped', function(reason)
	...
	if xPlayer then
		...
		for k,v in ipairs(GetPlayerIdentifiers(playerId)) do
			if string.match(v, 'license:') or string.match(v, 'Char1:') or string.match(v, 'Char2:') or string.match(v, 'Char3:') or string.match(v, 'Char4:')then
				identifier = v
				break
			end
		end
		TriggerEvent('esx_paycheck:ClockOutAuto', identifier)

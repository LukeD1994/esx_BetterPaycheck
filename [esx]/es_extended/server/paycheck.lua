ESX.StartPayCheck = function()
	...
	AddEventHandler('esx_paycheck:ClockIn', function()
		...
		MySQL.Async.execute('UPDATE `users` SET `clockin` = @time WHERE identifier = @identifier', {
			['@identifier'] = identifier,
			['@time'] = GetGameTimer()
		}, function(rowsChanged)
			--done
			print('player clocked in at: '..GetGameTimer())
		end)
	end)
	...
	AddEventHandler('esx_paycheck:ClockOut', function()
		...
		MySQL.Async.execute('UPDATE `users` SET `clockout` = @time WHERE identifier = @identifier', {
			...
			print('player clocked out at: '..GetGameTimer())
		end)

		local xPlayers = ESX.GetPlayerFromIdentifier(identifier)
		if xPlayer ~= nil then
			-- calculate hours worked
			MySQL.Async.fetchScalar('SELECT clockin, clockout FROM users WHERE identifier = @identifier',{
				['@identifier'] = identifier
			}, function(result)
				...
				if differenceMs ~= nil then
					...
					local payout = math.floor(differenceHrs) * salary
